...
public class Paddle {
	...
	private void setBounds(){
		if(player == 1){
			bounds = new Rectangle((int)position.getI()+(getWidth()-2),(int)position.getJ(),2,getHeight());
		}else{
	...
	public void checkCollisions(){
		...
		//if the paddle is touching the bottom of the screen
		if(position.getJ()+getHeight() >= screenSize[1]){
			position.setComponents(position.getI(), screenSize[1]-getHeight());
		}
		//if the paddle is touching the top of the screen
		if(position.getJ() <= 0){
			position.setComponents(position.getI(),0);
		}
	}
