...
import menu.MainMenu.MAIN;
import menu.MainMenu.PAUSE;
...
public class Game extends Canvas implements Runnable {
	...
	public void init() {
		menu = new MainMenu(panel.getWidth(), panel.getHeight(), paddle2);
	...
	public void update() {
		...
		if(gs == GAMESTATE.STARTMENU){
			...
			//if the gameState is 2 shut down the game
		...
		if(gs == GAMESTATE.PAUSE){
			menu.update(input);
			if(menu.ps == PAUSE.NULL){
				unPause();
				input.select.toggle(false);
			}else if(menu.ps == PAUSE.RESTART){
				reset();
				input.select.toggle(false);
			}
	...
	private void CPUMove(Paddle cpu) {
		...
		if (ball.getPosition().getI() > panel.getWidth() / 2) {
			...
			if (distance < 0 && distance > -menu.getDifficulty()) {
				cpu.setSpeed(distance);
			}
			
			if (distance < -menu.getDifficulty()) {
				cpu.setSpeed(-menu.getDifficulty());
			}
